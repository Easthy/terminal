<div class="white-content">
	...
	<div class="tab-container" style="max-height: 1200px;">
		<div class="tab-item no-margin text-centered">
            <p class="text-black-big-thick"><?= $model['name'] ?></p>
            <p class="text-blue-small-thick">Выберите даты посещения</p>
        </div>
        <div id="calendar-wrap" style="width:934px; height: 600px; margin-top: 30px;"></div>
        <div class="tab-item no-margin text-centered" style="padding: 32px;">
            <div style="width: 380px; display:inline-block;">
                <div class="check-profile text-left"></div>
                <div class="text-black-small text-centered" style="width: 200px; height:80px;">Посещать все</div>
            </div>
        </div>
	</div>
    ...
    <div class="btn-container">
        <a href="/activity/confirm" class="btn btn-green btn-small text-black-small">
            Подтвердить
        </a>
    </div>
</div>
...
<style type="text/css">
    ...
    .calendar-cell .calendar-active-day {
        color: #00cc99;
    }
...
class Calendar
{
    constructor(id)
    {
        ...
        this.active_week_days = <?= $active_week_days ?>;
    ...
    createCalendarBody(date, are_real_date_displayed) //returns a table fulfilled with days
    {
        ...
        for (var j = 0; j < 6; ++j)
        {
            ...
            for (var k = 0; k < 7; ++k)
            {
                ...
                } else if(this.active_week_days.includes(k+1)) {
                    var td_class = 'calendar-active-day';
                } else {
                    var td_class = '';
                }
...
$(function() {
    $('.check-profile').click(function() {
        if($('.check-profile').hasClass('active')) {
            $('.check-profile').removeClass('active');
        } else {
            $('.check-profile').addClass('active');
        }
    });
});
